class RegisterApiEntrepriseEnrollment < RegisterEnrollmentService
  ...
  def call
    ...
    register_enrollment_in_api_scopes(@enrollment.id.to_s, linked_token_manager_id, 'api_entreprise', scopes)
  end
  ...
  def create_enrollment_in_token_manager(name, email, scopes, contacts)
    ...
    # 1. create user
    formated_contacts = contacts
                            .select {|contact| contact['id'].in?(['technique', 'metier'])}
                            .map do |contact|
      {
          'email' => contact['email'],
    ...
    create_user_response = Http.post(
        ...
        "{\"email\": #{email.to_json},\"contacts\": #{formated_contacts.to_json}}",
        {'Authorization' => "Bearer #{api_key}"}
    )

    # 1. b) catch user already exists error
    # TODO entreprise: to avoid this, do not return an error if the user exists
    if create_user_response.code != '201' and not (create_user_response.code == '422' and JSON.parse(create_user_response.body)['errors']['email'].first == "value already exists")
